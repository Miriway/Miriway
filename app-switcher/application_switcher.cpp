...
// Imported from miral for investigation purposes (namespace changed from miral to miriway)
...
class miriway::ApplicationSwitcher::Self
{
...
miriway::ApplicationSwitcher::ApplicationSwitcher()
    : self(std::make_shared<Self>())
{
}

miriway::ApplicationSwitcher::~ApplicationSwitcher() = default;

miriway::ApplicationSwitcher::ApplicationSwitcher(ApplicationSwitcher const&) = default;

miriway::ApplicationSwitcher& miriway::ApplicationSwitcher::operator=(ApplicationSwitcher const&) = default;

void miriway::ApplicationSwitcher::operator()(wl_display* display)
{
    self->run_client(display);
}

void miriway::ApplicationSwitcher::operator()(std::weak_ptr<mir::scene::Session> const&) const
{
}

void miriway::ApplicationSwitcher::next_app() const
{
    self->next_app();
}

void miriway::ApplicationSwitcher::prev_app() const
{
    self->prev_app();
}

void miriway::ApplicationSwitcher::next_window() const
{
    self->next_window();
}

void miriway::ApplicationSwitcher::prev_window() const
{
    self->prev_window();
}

void miriway::ApplicationSwitcher::confirm() const
{
    self->confirm();
}

void miriway::ApplicationSwitcher::cancel() const
{
    self->cancel();
}

void miriway::ApplicationSwitcher::stop() const
{
    self->stop();
}

class miriway::BasicApplicationSwitcher::Self
{
...
public:
    explicit Self(KeybindConfiguration const& keybind_configuration)
        : startup_internal_client(switcher, switcher, [switcher=switcher] { switcher.stop(); }),
         ...
         {
            auto const modifiers = miral::toolkit::mir_keyboard_event_modifiers(key_event);
            if (miral::toolkit::mir_keyboard_event_action(key_event) == mir_keyboard_action_down)
            {
    ...
    miral::AppendKeyboardEventFilter keyboard_event_filter;
};

miriway::BasicApplicationSwitcher::BasicApplicationSwitcher()
    : BasicApplicationSwitcher(KeybindConfiguration{}) {}

miriway::BasicApplicationSwitcher::BasicApplicationSwitcher(KeybindConfiguration const& keybind_configuration)
    : self(std::make_shared<Self>(keybind_configuration))
{
}

miriway::BasicApplicationSwitcher::~BasicApplicationSwitcher() = default;

void miriway::BasicApplicationSwitcher::operator()(mir::Server& server) const
{
